...
export default function SymptomAnalysis() {
  ...
  const [phase, setPhase] = useState('input');   // input | analyzing | chat | result | emergency
  ...
  const handleAnalyze = async () => {
    ...
    const newHistory = [{ role: 'user', content: symptoms.trim() }];
    setHistory(newHistory);
    try {
      const res = await aiAPI.chat({ history: newHistory });
      const data = res.data;
  ...
  const handleReset = () => { setSymptoms(''); setHistory([]); setChatInput(''); setPhase('input'); setResult(null); setError(''); };
          ...
          <AnimatePresence mode="wait">
            ...
            {phase === 'chat' && (
              <motion.div key="chat" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 460 }}>
                {/* Chat History Header */}
                <div style={{ padding: '24px 32px 16px', borderBottom: `1px solid ${P.border}` }}>
                  <h2 style={{ fontSize: 18, fontWeight: 800, color: P.ink, margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
                    <Brain size={18} color={P.teal} /> AI Clarification
                  </h2>
                  <p style={{ fontSize: 13, color: P.sub, margin: '4px 0 0' }}>The AI needs more details to accurately match a specialist.</p>
                </div>
                ...
                <div style={{ flex: 1, padding: '24px 32px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
                  {history.map((msg, i) => (
                    <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', alignSelf: msg.role === 'user' ? 'flex-end' : 'flex-start', maxWidth: '85%' }}>
                      {msg.role === 'assistant' && (
                        <div style={{ width: 32, height: 32, borderRadius: 10, background: `${P.teal}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          <Brain size={15} color={P.teal} />
                        </div>
                      )}
                      <div style={{
                        padding: '12px 16px', borderRadius: 16, fontSize: 13.5, lineHeight: 1.5,
                        background: msg.role === 'user' ? `linear-gradient(135deg, ${P.navy}, ${P.navyLt})` : P.sky,
                        color: msg.role === 'user' ? 'white' : P.ink,
                ...
                {/* Input Area */}
                <div style={{ padding: '24px 32px', borderTop: `1px solid ${P.border}`, background: P.white }}>
                  <form onSubmit={handleChatReply} style={{ display: 'flex', gap: 10 }}>
                    <input autoFocus value={chatInput} onChange={e => setChatInput(e.target.value)}
                      placeholder="Type your answer..."
                      style={{ flex: 1, padding: '12px 16px', borderRadius: 12, border: `1px solid ${P.border}`, background: P.sky, outline: 'none', fontSize: 14, color: P.ink, fontFamily: 'inherit' }} />
                    <button type="submit" disabled={!chatInput.trim()}
                      style={{ width: 44, height: 44, borderRadius: 12, border: 'none', background: chatInput.trim() ? `linear-gradient(135deg, ${P.teal}, ${P.tealDk})` : 'rgba(0,0,0,0.06)', color: chatInput.trim() ? 'white' : P.muted, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: chatInput.trim() ? 'pointer' : 'not-allowed', transition: 'all 0.2s' }}>
                      <Send size={16} />
                    </button>
                  </form>
                  <p style={{ fontSize: 11, color: P.muted, margin: '10px 0 0', textAlign: 'center' }}>Maximum 3 messages per session.</p>
                </div>
              </motion.div>
            )}
            ...
            {phase === 'result' && result && (
              <motion.div key="result" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ padding: '32px' }}>
                ...
                {/* Urgency banner */}
                <div style={{ padding: '16px 20px', borderRadius: 16, background: urgStyle.bg, border: `1.5px solid ${urgStyle.color}22`, marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
