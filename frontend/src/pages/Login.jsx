...
function Input({ icon: Icon, label, type = 'text', value, onChange, placeholder, error, suffix }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', borderRadius: 14, background: focused ? 'rgba(255,255,255,0.95)' : 'rgba(255,255,255,0.72)', border: `1.5px solid ${error ? P.error : focused ? P.teal : 'rgba(15,23,42,0.08)'}`, transition: 'all 0.3s', backdropFilter: 'blur(10px)', boxShadow: focused ? '0 10px 24px rgba(15, 23, 42, 0.1)' : 'none' }}>
        <Icon size={15} color={focused ? P.teal : P.muted} style={{ flexShrink: 0, transition: 'color 0.3s' }} />
        <input type={type} value={value} onChange={onChange} placeholder={placeholder}
          onFocus={() => setFocused(true)} onBlur={() => setFocused(false)}
          style={{ flex: 1, border: 'none', background: 'transparent', outline: 'none', fontSize: 14, color: P.ink, fontFamily: "var(--font-sans)", fontWeight: 500 }} />
        {suffix}
      </div>
      {error && (
      ...
      {[{ value: 'patient', label: 'Patient', icon: User }, { value: 'doctor', label: 'Doctor', icon: Stethoscope }].map(({ value, label, icon: Icon }) => {
        ...
        return (
          <motion.button key={value} onClick={() => onChange(value)} whileTap={{ scale: 0.97 }}
            style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, padding: '10px', borderRadius: 10, border: 'none', cursor: 'pointer', fontFamily: "var(--font-sans)", fontSize: 13, fontWeight: 700, background: active ? `linear-gradient(135deg, ${P.teal}, ${P.tealDk})` : 'transparent', color: active ? 'white' : P.muted, boxShadow: active ? '0 4px 16px rgba(13,196,161,0.25)' : 'none', transition: 'all 0.3s' }}>
            <Icon size={14} />{label}
          </motion.button>
        );
      })}
...
export default function Login() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (status === 403 && payload?.verificationStatus) {
        setPendingStatus(payload.verificationStatus);
        setRejectionReason(payload.rejectionReason || '');
        return;
      }
      setError(payload?.message || payload?.error || 'Invalid credentials. Please try again.');
    } finally {
      setLoading(false);
    }
  };
...
if (pendingStatus) {
  return (
    <div style={{ minHeight: '100vh', background: 'var(--mc-sky)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-sans)', padding: 16 }}>
      <motion.div
        initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        style={{
          maxWidth: 460, width: '100%', textAlign: 'center',
          background: 'rgba(255,255,255,0.82)', backdropFilter: 'blur(26px)',
  ...
  return (
    <div style={{ fontFamily: "var(--font-sans)", minHeight: '100vh', display: 'flex', background: 'var(--mc-sky)', overflow: 'hidden', position: 'relative' }}>
      ...
      <motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, ease: ease }}
        style={{ width: '42%', background: `linear-gradient(155deg, ${P.navy} 0%, ${P.navyLt} 100%)`, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '64px 56px', position: 'relative', overflow: 'hidden', zIndex: 1 }}>
          ...
          <div style={{ borderRadius: 28, padding: '44px 40px', background: 'rgba(255,255,255,0.65)', backdropFilter: 'blur(24px) saturate(180%)', border: '1px solid rgba(255,255,255,0.7)', boxShadow: '0 24px 80px -12px rgba(11,37,69,0.12)' }}>
            ...
            {showForgotPass ? (
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                ...
                {forgotMessage && (
                  <div style={{ marginBottom: 20, padding: '12px 14px', borderRadius: 12, background: 'rgba(13,196,161,0.08)', border: '1px solid rgba(13,196,161,0.2)' }}>
                    <p style={{ margin: 0, fontSize: 13, color: P.tealDk, fontWeight: 600 }}>{forgotMessage}</p>
                  </div>
                )}
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                  onClick={handleForgotPassword} disabled={loading}
                  style={{ width: '100%', padding: '14px', borderRadius: 16, border: 'none', background: loading ? `${P.teal}80` : `linear-gradient(135deg, ${P.teal}, ${P.tealDk})`, color: 'white', fontWeight: 800, fontSize: 15, cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'Inter' }}>
                  {loading ? 'Sending...' : 'Send Reset Link'}
                </motion.button>
                <button onClick={() => { setShowForgotPass(false); setError(''); setForgotMessage(''); setFieldErr({}); }}
                  style={{ width: '100%', marginTop: 12, padding: '14px', borderRadius: 16, border: '1.5px solid rgba(11,37,69,0.1)', background: 'transparent', color: P.sub, fontWeight: 600, fontSize: 14, cursor: 'pointer', fontFamily: 'Inter' }}>
                  Back to Log In
                </button>
              </motion.div>
            ) : (
              ...
              <Input icon={Lock} label="Password" type={showPass ? 'text' : 'password'} value={password}
                ...
                <div style={{ textAlign: 'right' }}>
                  <button onClick={() => { navigate('/forgot-password'); }}
                    style={{ background: 'none', border: 'none', color: P.teal, fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'Inter', padding: 0 }}>
                    Forgot Password?
                  </button>
                </div>
              </div>
